...
void unphysical_masses(bool saveoutput=true, bool close=false, bool daughter=false){

	//****Input
	TString inPath = "/home/ikp1/puetz/panda/mysimulations/analysis/pbarp_lambda0_antilambda0/10000_events/idealtracking/";
	...
	ntpLambda0->Project("h_m_moth", "VtxFit_m", "McTruthMatch==1");

	TH1D * h_mreco_moth = new TH1D ("h_mreco_moth", "Mass of Lambda0 before Fit; m/ GeV/c^{2}; counts", 100, 1,1.22);
	ntpLambda0->Project("h_mreco_moth", "Lambda0_m", "McTruthMatch==1");

	TH1D * h_mdiff_moth = new TH1D ("h_mdiff_moth", "Mass diff of Lambda0; m_{reco}-m_{fit}/ GeV/c^{2}; counts", 100, -0.2,0.2);
	ntpLambda0->Project("h_mdiff_moth", "Lambda0_m-VtxFit_m", "McTruthMatch==1");

	TH1D * h_p_moth = new TH1D ("h_p_moth", "Momentum of Lambda0 after Fit; m/ GeV/c^{2}; counts", 100, 0,5);
	ntpLambda0->Project("h_p_moth", "VtxFit_p", "McTruthMatch==1");

	TH1D * h_preco_moth = new TH1D ("h_preco_moth", "Mometum of Lambda0 before Fit; m/ GeV/c^{2}; counts", 100, 0,5);
	ntpLambda0->Project("h_preco_moth", "Lambda0_p", "McTruthMatch==1");

	TH1D * h_pdiff_moth = new TH1D ("h_pdiff_moth", "Momentum diff of Lambda0; p_{reco}-p_{fit}/ GeV/c; counts", 100, -0.2,0.2);
	ntpLambda0->Project("h_pdiff_moth", "Lambda0_p-VtxFit_p", "McTruthMatch==1");

	TH1D * h_e_moth = new TH1D ("h_e_moth", "Energy of Lambda0 after Fit; m/ GeV/c^{2}; counts", 100, 0,5);
	ntpLambda0->Project("h_e_moth", "VtxFit_e", "McTruthMatch==1");

	TH1D * h_ereco_moth = new TH1D ("h_ereco_moth", "Energy of Lambda0 before Fit; m/ GeV/c^{2}; counts", 100, 0,5);
	ntpLambda0->Project("h_ereco_moth", "Lambda0_e", "McTruthMatch==1");

	TH1D * h_ediff_moth = new TH1D ("h_ediff_moth", "Energy diff of Lambda0; e_{reco}-e_{fit}/ GeV; counts", 100, -0.2,0.2);
	ntpLambda0->Project("h_ediff_moth", "Lambda0_e-VtxFit_e", "McTruthMatch==1");

//	jenny::CreateDrawAndSaveHistogram(h_mreco_moth, outPath, "h_mreco_moth1", saveoutput, close);
	jenny::CreateDrawAndSaveNHistograms(h_mreco_moth, h_m_moth, "before fit", "after fit", outPath, "h_m_before_and_after_fit", saveoutput, close);
