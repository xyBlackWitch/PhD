...
void vertex_studies_XiMinus(int nevts=0, bool saveoutput=true, bool close=false){
	...
	TString inputFile = inPath + "output_ana_oldidealtracking.root";
	...
	TH1D* h_vx_mc = new TH1D("h_vx_mc", "x coordinate of the vertex position; x[cm]; counts", 50,0,5);
	ntpMC->Project("h_vx_mc", "x", "moth==0 && pdg==3312");

	TH1D* h_vy_mc = new TH1D("h_vy_mc", "y coordinate of the vertex position; y[cm]; counts", 50,0,5);
	ntpMC->Project("h_vy_mc", "y", "moth==0 && pdg==3312");

	TH1D* h_vz_mc = new TH1D("h_vz_mc", "z coordinate of the vertex position; z[cm]; counts", 50,0,5);
	...
	TH1D* h_vx_res = new TH1D("h_vx_res", "x resolution of the vertex position; #Delta_{x}[cm]; counts", 100,-2,2);
	ntpXiMinus->Project("h_vx_res", "XiMinusFit_vx-McTruth_d0_x", "McTruthMatch==1 ");

	TH1D* h_vy_res = new TH1D("h_vy_res", "y resolution of the vertex position; #Delta_{y}[cm]; counts", 100,-2,2);
	ntpXiMinus->Project("h_vy_res", "XiMinusFit_vy-McTruth_d0_y", "McTruthMatch==1 ");

	TH1D* h_vz_res = new TH1D("h_vz_res", "z resolution of the vertex position; #Delta_{z}[cm]; counts", 100,-2,2);
	ntpXiMinus->Project("h_vz_res", "XiMinusFit_vz-McTruth_d0_z", "McTruthMatch==1");
	...
	TH1D* h_vz_poc_res = new TH1D("h_vz_poc_res", "differenz reconstructed vertex position and poca position; z_{reco} - z_{poca}[cm]; counts", 100,-2,2);
	...
	TH1D* h_reco_div_mc_x = new TH1D("h_reco_div_mc_x", "ratio reco/MC for x position of vertex; x[cm]; ratio reco/MC", 100,-10,10);
	h_reco_div_mc_x->Divide(h_vx_reco, h_vx_mc);
	h_reco_div_mc_x->SetStats(0);

	TH1D* h_reco_div_mc_y = new TH1D("h_reco_div_mc_y", "ratio reco/MC for y position of vertex; y[cm]; ratio reco/MC", 100,-10,10);
	h_reco_div_mc_y->Divide(h_vy_reco, h_vy_mc);
	h_reco_div_mc_y->SetStats(0);

	TH1D* h_reco_div_mc_z = new TH1D("h_reco_div_mc_z", "ratio reco/MC for z position of vertex; z[cm]; ratio reco/MC", 100,-10,10);
	...
	TH1D * h_chi2 = new TH1D("h_chi2", "#chi^{2} distribution for vertex Fit; #chi^{2}; counts", 50,-10,10);
	...
	jenny::CreateDrawAndSaveHistogram(h_vx_poc_res, outPath, "h_vx_poc_res", saveoutput, close);
	jenny::CreateDrawAndSaveHistogram(h_vy_poc_res, outPath, "h_vy_poc_res", saveoutput, close);
	jenny::CreateDrawAndSaveHistogram(h_vz_poc_res, outPath, "h_vz_poc_res", saveoutput, close);
