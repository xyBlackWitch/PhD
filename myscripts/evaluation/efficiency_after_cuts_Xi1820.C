...
void efficiency_after_cuts_Xi1820(TString oldtrunk="", TString newtrunk="", TString ntuple="ntpXiMinus1820", TString value="XiMinus"){
	...
	gStyle->SetOptStat(0);
	//*** Create Histogram
	TH1D * h_old = new TH1D("h_old", "Efficiency after each cut ; ; efficiency [%]", 2,0,2);
	...
	TH1D * h_new = new TH1D("h_new", "Efficiency after each cut; ; efficiency [%]", 2,0,2);
