...
void eval_Xi(TString path="", bool save=kTRUE, bool close=kFALSE){
	...
	if(ntpXiMinus!=0X0){
		...
		TH1D * h_xi_vtxres_x = new TH1D("h_xi_vtxres_x", " ; x-x_{MC}; counts", 500,-1,1);
		...
		TH1D * h_xi_vtxres_y = new TH1D("h_xi_vtxres_y", " ; y-y_{MC}; counts", 500,-1,1);
		...
		TH1D * h_xi_vtxres_z = new TH1D("h_xi_vtxres_z", " ; z-z_{MC}; counts", 500,-1,1);
...
//		jenny::CreateDrawAndSaveNHistograms(h_axi_m_nocut, h_axi_m_vtxcut, h_axi_m_masscut, "mass window", "VtxFit_prob>0.01", "VtxFit_prob>0.01 && MassFit_prob>0.01", path+"/plots/", "XiPlus_m_diffcuts", save, close);
		...
		jenny::CreateDrawAndSaveHistogramDoulbeFit(h_axi_m_masscut2, path+"/plots/", "XiPlus_m_masscut", save, close, false, 0.01,0.1);
