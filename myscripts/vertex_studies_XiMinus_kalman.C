...
void vertex_studies_XiMinus_kalman(int nevts=0, bool saveoutput=true, bool close=false){
	...
	TH1D* h_vx_res = new TH1D("h_vx_res", "x resolution of the vertex position; #Delta x[cm]; counts", 100,-1,1);
	ntpXiMinus->Project("h_vx_res", "KalmanFit_vx-MCTruth_vx", "McTruthMatch==1 ");

	TH1D* h_vy_res = new TH1D("h_vy_res", "y resolution of the vertex position; #Delta y[cm]; counts", 100,-1,1);
	ntpXiMinus->Project("h_vy_res", "KalmanFit_vy-MCTruth_vy", "McTruthMatch==1 ");

	TH1D* h_vz_res = new TH1D("h_vz_res", "z resolution of the vertex position; #Delta z[cm]; counts", 100,-1.5,1.5);
	...
	jenny::CreateDrawAndSaveHistogramWithFit(h_vx_res, outPath, "h_vx_res_fit_kalman", saveoutput, close, noAutorange, 0.03, 1);
	jenny::CreateDrawAndSaveHistogramWithFit(h_vy_res, outPath, "h_vy_res_fit_kalman", saveoutput, close, noAutorange, 0.03, 1);
	jenny::CreateDrawAndSaveHistogramWithFit(h_vz_res, outPath, "h_vz_res_fit_kalman", saveoutput, close, noAutorange, 0.015, 1.5);
...
//	jenny::CreateDrawAndSaveHistogram(h_vz_res, outPath, "h_vz_res_kalman", saveoutput, close);
	...
	jenny::CreateDrawAndSaveHistogramWithFit(h_vx_poc_res, outPath, "h_vx_poc_res_kalman", saveoutput, close);//, noAutorange, 0.16, 1);
	jenny::CreateDrawAndSaveHistogramWithFit(h_vy_poc_res, outPath, "h_vy_poc_res_kalman", saveoutput, close, noAutorange, 0.02, 2);
	jenny::CreateDrawAndSaveHistogramWithFit(h_vz_poc_res, outPath, "h_vz_poc_res_kalman", saveoutput, close, noAutorange, 0.15,5);
