...
void reco_complete_GenFit2(TString pre="")
{
  ...
  // tracking->Cleanup();
  tracking->SetPersistence(kFALSE); //era commentato via nel genfit test
  fRun->AddTask(tracking);
  
  PndSttMvdGemTracking * SttMvdGemTracking = new PndSttMvdGemTracking(0);
  //SttMvdGemTracking->SetPdgFromMC();
  SttMvdGemTracking->SetPersistence(kFALSE);//era commentato via nel genfit test
  ...
  trackMC->SetPersistence(kFALSE);//era commentato via nel genfit test
  ...
  trackFts->SetPersistence(kFALSE);//era commentato via nel genfit test
