...
//  fRun->AddTask(tracking);
  /*
  PndSttMvdGemTrackingIdeal * SttMvdGemTracking = new PndSttMvdGemTrackingIdeal();
  //SttMvdGemTracking->SetPdgFromMC();
  SttMvdGemTracking->SetRelativeMomentumSmearing(0.05);
  SttMvdGemTracking->SetVertexSmearing(0.05, 0.05, 0.05);
  SttMvdGemTracking->SetTrackingEfficiency(1.);
  SttMvdGemTracking->SetTrackOutput("SttMvdGemTrack");
  //SttMvdGemTracking->SetPersistence(kFALSE);
  fRun->AddTask(SttMvdGemTracking);
  */
  PndMCIdealTrackFinderNewLinks * idealTrack = new PndMCIdealTrackFinderNewLinks();
  fRun->AddTask(idealTrack);
  ...
  recoKalman->SetTrackInBranchName("IdealTrack");
  recoKalman->SetTrackInIDBranchName("IdealTrackID");
  recoKalman->SetTrackOutBranchName("IdealGenTrack");
  ...
  trackMC2->SetTrackInBranchName("IdealGenTrack");
  trackMC2->SetTrackOutBranchName("IdealGenTrackID");
  fRun->AddTask(trackMC2);

  PndRecoKalmanTask2* recoKalman2 = new PndRecoKalmanTask2();
  recoKalman2->SetTrackInBranchName("IdealTrack");
  recoKalman2->SetTrackInIDBranchName("IdealTrackID");
  recoKalman2->SetTrackOutBranchName("IdealGenTrack2");
  ...
  PndMCTrackAssociator* trackMCGenfit2 = new PndMCTrackAssociator();
  trackMCGenfit2->SetTrackInBranchName("IdealGenTrack2");
  trackMCGenfit2->SetTrackOutBranchName("IdealGenTrack2ID");
  fRun->AddTask(trackMCGenfit2);
