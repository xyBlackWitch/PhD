...
sim_complete_boxgen_6fs(Int_t nEvents = 100, Float_t mom = 6.231552, TString pre="", double min, double max, TString  SimEngine ="TGeant3")
{
  //-----User Settings:-----------------------------------------------
  ...
  if(UseBoxGenerator){	// Box Generator
    ...
    boxGenPiM->SetPRange(mom,mom); // GeV/c
    boxGenPiM->SetPhiRange(0., 360.); // Azimuth angle range [degree]
    boxGenPiM->SetThetaRange(min, max); // Polar angle in lab system range [degree]
	...
	boxGenPiP->SetPRange(mom,mom); // GeV/c
	boxGenPiP->SetPhiRange(0., 360.); // Azimuth angle range [degree]
	boxGenPiP->SetThetaRange(min, max); // Polar angle in lab system range [degree]
	boxGenPiP->SetXYZ(0., 0., 0.); // cm
	primGen->AddGenerator(boxGenPiP);

    FairBoxGenerator* boxGenProt = new FairBoxGenerator(2212, 1); // 13 = muon; 1 = multipl.
	boxGenProt->SetPRange(mom,mom); // GeV/c
	boxGenProt->SetPhiRange(0., 360.); // Azimuth angle range [degree]
	boxGenProt->SetThetaRange(min, max); // Polar angle in lab system range [degree]
	boxGenProt->SetXYZ(0., 0., 0.); // cm
	primGen->AddGenerator(boxGenProt);

    FairBoxGenerator* boxGenAProt = new FairBoxGenerator(-2212, 1); // 13 = muon; 1 = multipl.
	boxGenAProt->SetPRange(mom,mom); // GeV/c
	boxGenAProt->SetPhiRange(0., 360.); // Azimuth angle range [degree]
	boxGenAProt->SetThetaRange(min, max); // Polar angle in lab system range [degree]
	boxGenAProt->SetXYZ(0., 0., 0.); // cm
	primGen->AddGenerator(boxGenAProt);

    FairBoxGenerator* boxGenKM = new FairBoxGenerator(-321, 1); // 13 = muon; 1 = multipl.
	boxGenKM->SetPRange(mom,mom); // GeV/c
	boxGenKM->SetPhiRange(0., 360.); // Azimuth angle range [degree]
	boxGenKM->SetThetaRange(min, max); // Polar angle in lab system range [degree]
	...
	boxGenKP->SetPRange(mom,mom); // GeV/c
	boxGenKP->SetPhiRange(0., 360.); // Azimuth angle range [degree]
	boxGenKP->SetThetaRange(min, max); // Polar angle in lab system range [degree]
