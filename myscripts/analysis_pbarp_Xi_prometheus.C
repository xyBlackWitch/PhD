...
void analysis_pbarp_Xi_prometheus(TString pre="", int nevts=0){
...
//  TString outPath = " ";
  ...
  TString inPIDFile = pre + "_pid.root"; //Path + "pid_complete.root";
  TString inParFile = pre + "_par.root"; //Path + "simparams.root";
  ...
  //Create output file 
  TFile *out = TFile::Open(pre + "output_ana.root","RECREATE");
...
//    theAnalysis->FillList(NotCombinedPiPlus, "PionAllPlus", PidSelection);
    ...
    for (int pip=0; pip<piplus.GetLength(); ++pip){
        ...
        ntpPiPlus->Column("McTruthMatch", (Bool_t) theAnalysis->McTruthMatch(piplus[pip]));
