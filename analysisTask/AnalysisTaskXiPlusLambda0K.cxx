...
#include "AnalysisTaskXiPlusLambda0K.h"
...
ClassImp(AnalysisTaskXiPlusLambda0K)

//---------------Default constructor------------
AnalysisTaskXiPlusLambda0K::AnalysisTaskXiPlusLambda0K():
...
void AnalysisTaskXiPlusLambda0K::CombinedList(RhoCandidate* cand, RhoCandList* combinedList, int pdg){
	  /**
...
void AnalysisTaskXiPlusLambda0K::GetNotCombinedList(RhoCandList combinedList, RhoCandList * candList){
...
void AnalysisTaskXiPlusLambda0K::numberOfHitsInSubdetector(TString pre, RhoCandidate *c, RhoTuple *n){
...
void AnalysisTaskXiPlusLambda0K::tagNHits(TString pre, RhoCandidate *c, RhoTuple *n){
...
int AnalysisTaskXiPlusLambda0K::tagHits(RhoCandidate *c){
...
std::map<int,int> AnalysisTaskXiPlusLambda0K::VertexQaIndex(RhoCandList* candList, float probLimit=0.01){
...
std::map<int,int> AnalysisTaskXiPlusLambda0K::MassFitQaIndex(RhoCandList* candList, float m0, float probLimit=0.01){
...
void AnalysisTaskXiPlusLambda0K::qaVtxDiff(TString pre, RhoCandidate * c, RhoTuple * n){
...
void AnalysisTaskXiPlusLambda0K::qaMomRes(TString pre, RhoCandidate * c, RhoTuple * n){
...
InitStatus AnalysisTaskXiPlusLambda0K::Init(){
...
void AnalysisTaskXiPlusLambda0K::Exec(Option_t* op)
{
	 //RhoCandLists for analysis
	RhoCandList piplus, piminus, lambda0, antiLambda0, proton, antiProton, kaonminus, kaonplus, xiplus, xiSys;
	RhoCandList CombinedPiPlus, NotCombinedPiPlus; 
	RhoCandList Lambda0Fit, AntiLambda0Fit, XiPlusFit, KaonMinusCand;
	...
	while (fAnalysis->GetEvent() && ++fEvtCount<fnevts){
	    ...
	    for (int k=0; k<kaonminus.GetLength(); ++k){
			...
			if(tagHits(kaonminus[k])){
				KaonMinusCand.Append(kaonminus[k]);
			} 
...
//	    ******* Xi+ Xi- System*****************************
	     ...
	     xiSys.Combine(XiPlusFit, Lambda0Fit, KaonMinusCand);
	     xiSys.SetType(88888);
	     ...
	     Lambda0Fit.Cleanup();
	     XiPlusFit.Cleanup();
	     KaonMinusCand.Cleanup();

   }
}

void AnalysisTaskXiPlusLambda0K::Finish()
{
